...
class Account:
    __slots__ = ("username", "uuid", "access_token", "client_token")

    def __init__(self, username: str, uuid: McUUID, access_token: str | UUID, client_token: str | None) -> None:
    ...
    @classmethod
    async def _from_xbox_access_token(cls, client: httpx.AsyncClient, access_token: str) -> Self:
        ...
        return cls(data["name"], McUUID(data["id"]), access_token, None)
    ...
    async def xbox_auth(cls, client: httpx.AsyncClient, user_hash: str, xsts_token: str) -> Self:
        ...
        return await cls._from_xbox_access_token(client, access_token)
